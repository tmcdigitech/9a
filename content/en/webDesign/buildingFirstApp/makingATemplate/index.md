...
In our project folder, alongside app.py, make a new folder called `templates`. If you need to, it is possible to configure Flask so it'll get templates from a different folder, but the default (*templates* in the same folder as the app.py file) works fine for us! Make a new file in it and call it `demo.html`.

![Our first template!](firstTemplate.png)

We're now going to move all that HTML out of our demo function and into the demo template. Below are the file contents of the two files now. Note that in app.py we now import a Flask function called `render_template` on line 1, and we call it in our demo function on line 18.

### `app.py`
```python{linenos=table,hl_lines=[1,18]}
